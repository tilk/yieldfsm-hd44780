...
module FSM.HD44780(controller, module FSM.HD44780.Definitions) where
...
[fsm|controller :: forall dom period. (HiddenClockResetEnable dom, DomainPeriod dom ~ period, KnownNat period, 1 <= period)
                => Signal dom Bus_Input
                -> Signal dom (Data_Input, Bus_Output)
input bus
fun sendbyte (rs, d):
